<?php
	...
	if(isset($_POST) && array_key_exists('Login',$_POST)){
		...
		$result = mysqli_query($link, "SELECT `user_ID`,`password`,`first_name`,`username`,`email`,`middle_name`,`last_name`,`unit`,`unit_type`,`user_level`,`approval` FROM ".DB_PREFIX."system_users WHERE $user_cond") or die (mysqli_error()); 
		$num = mysqli_num_rows($result);

		// Match row found with more than 1 results  - the user is authenticated. 
		if ( $num > 0 ) { 
			list($id,$pwd,$first_name,$username,$email,$middle_name,$last_name,$unit,$unit_type,$userlevel,$approved) = mysqli_fetch_row($result);	
			if($approved == 0) {
				$_SESSION['error_msg'] = "Account inactive. Please contact the administrator";
